...
def _fsm_set_alt_hold():
    ...
    if master is None:
        print("[AUTO] ALT_HOLD gagal — Pixhawk belum terhubung")
        return False
    ...
    try:
        ...
        if "ALT_HOLD" not in mode_mapping:
            print("[AUTO] ALT_HOLD tidak tersedia di firmware")
            return False

        with master_lock:
            master.set_mode(mode_mapping["ALT_HOLD"])

        requested_mode = "ALT_HOLD"
        requested_mode_ts = time.time()

        print("[AUTO] Request mode -> ALT_HOLD")
        return True

    except Exception as e:
        print(f"[AUTO] Gagal set ALT_HOLD: {e}")
        return False
