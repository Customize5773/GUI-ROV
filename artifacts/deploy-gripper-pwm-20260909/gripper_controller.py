...
class GripperController:
    ...
    def set_pwm(self, pwm):
        """PWM SERVO7 langsung, dengan batas mekanik dan slew yang sama."""
        if (type(pwm) not in (int, float) or not math.isfinite(pwm)
                or not min(GRIPPER_PWM_CLOSE, GRIPPER_PWM_OPEN) <= pwm <= max(GRIPPER_PWM_CLOSE, GRIPPER_PWM_OPEN)):
            raise ValueError("PWM gripper di luar rentang mekanik")
        self._set_target(pwm)
